...
// Chemistry kartlarını manuel olarak tanımla
...
// Chemistry kartlarını temizle ve yeniden yükle
async function clearAndUploadChemistryCards() {
  try {
    console.log('🚀 Chemistry kartları temizleniyor ve yeniden yükleniyor...');
    ...
    // Mevcut chemistry kartlarını sil
    console.log('🗑️ Mevcut chemistry kartları siliniyor...');
    for (const card of chemistryCards) {
      try {
        const deleteResponse = await fetch(`${API_BASE_URL}/cards/${card.id}`, {
          method: 'DELETE'
        });
        if (deleteResponse.ok) {
          console.log(`   ✅ ${card.id} silindi`);
        }
      } catch (error) {
        // Kart zaten yoksa hata vermez
      }
    }
    
    // Yeni chemistry kartlarını yükle
    console.log('📚 Yeni chemistry kartları yükleniyor...');
    ...
    // İlk birkaç kartı göster
    if (testData.data && testData.data.length > 0) {
      console.log('\n📋 İlk 3 chemistry kartı:');
      testData.data.slice(0, 3).forEach((card, index) => {
        console.log(`${index + 1}. ${card.question}`);
        console.log(`   Cevap: ${card.answer}`);
        console.log(`   Zorluk: ${card.difficulty}`);
        console.log('');
      });
    }
    
  } catch (error) {
    console.error('❌ Chemistry kartları işlenirken hata:', error.message);
    process.exit(1);
  }
}
...
if (require.main === module) {
  clearAndUploadChemistryCards();
}

module.exports = { clearAndUploadChemistryCards };
