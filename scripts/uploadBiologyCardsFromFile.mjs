...
async function uploadBiologyCards() {
  try {
    ...
    if (result.success) {
      console.log(`✅ ${result.insertedCount} biyoloji kartı başarıyla MongoDB'ye yüklendi!`);
      console.log('📊 Yüklenen kart sayısı:', result.insertedCount);
      console.log('📊 İlk 10 kart ID:', Object.values(result.insertedIds).slice(0, 10).join(', '));
    } else {
