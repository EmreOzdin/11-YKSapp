...
export const useAdaptiveFontSize = (
  ...
  if (deviceType.isSmall)
    return responsiveFontSize(baseSize * multiplier * 0.9);
  if (deviceType.isMedium) return responsiveFontSize(baseSize * multiplier);
  if (deviceType.isLarge)
    return responsiveFontSize(baseSize * multiplier * 1.1);
...
export const useAdaptiveHeight = (
  baseHeight: number,
  multiplier: number = 1
) => {
  const deviceType = useDeviceType();
  ...
  if (deviceType.isSmall)
    return responsiveHeight(baseHeight * multiplier * 0.8);
  if (deviceType.isMedium) return responsiveHeight(baseHeight * multiplier);
  if (deviceType.isLarge)
    return responsiveHeight(baseHeight * multiplier * 1.2);
