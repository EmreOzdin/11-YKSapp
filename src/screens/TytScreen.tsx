...
const TytScreen: React.FC = () => {
  ...
  const checkUserSelections = async () => {
    try {
      ...
      setHasSelections(selections?.examType === 'TYT' && selections.selectedTopics.length > 0);
    } catch (error) {
  ...
  const handleStartQuestions = async () => {
    if (!hasSelections) {
      Alert.alert(
        ...
        [
          {
            text: "Konu Seç",
            onPress: () => navigation.navigate('TopicSelectionScreen', { examType: 'TYT' })
          },
          {
            text: "İptal",
            style: "cancel"
    ...
    navigation.navigate('QuestionScreen', { examType: 'TYT' });
  };

  const handleTopicSelection = () => {
    navigation.navigate('TopicSelectionScreen', { examType: 'TYT' });
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        <Text style={styles.title}>TYT</Text>
        <Text style={styles.subtitle}>Temel Yeterlilik Testi</Text>
        
        <View style={styles.infoCard}>
          <Text style={styles.infoTitle}>TYT Hakkında</Text>
